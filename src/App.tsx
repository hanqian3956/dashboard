...
// 数据
const metalData = [
  { name: '铜', value: 12568.75, unit: '万吨' },
  { name: '铅', value: 3256.80, unit: '万吨' },
  { name: '锌', value: 4589.23, unit: '万吨' },
  { name: '镍', value: 1856.42, unit: '万吨' },
  { name: '钼', value: 986.55, unit: '万吨' },
  { name: '金', value: 1256.80, unit: '吨' },
  { name: '银', value: 8956.42, unit: '吨' },
  { name: '五氧化二钒', value: 3568.75, unit: '吨' },
  { name: '铁', value: 45236.88, unit: '万吨' },
];

const lakeData = [
  { name: '柯柯(NaCl)', value: 25680.55, unit: '万吨' },
  { name: '茶卡(NaCl)', value: 38956.72, unit: '万吨' },
  { name: '镁业(MgCl₂)', value: 12356.88, unit: '万吨' },
  { name: '东台(Li)', value: 5680.42, unit: '万吨' },
];
...
export default function App() {
  ...
  // 生成模拟数据 - 每个单位可能有多个矿种，包含品位信息
  const generateMineralData = (unit: string) => {
    // 定义各单位可能的矿种和对应的品位（红色标记的元素）
    const unitMinerals: Record<string, Array<{ mineral: string; grades: Record<string, number> }>> = {
      '锡铁山分公司': [
        { mineral: '铅锌', grades: { Pb: 4.52, Zn: 3.86, S: 12.05, Au: 2.3, Ag: 156.8 } },
      ],
      '西部铜业': [
        { mineral: '铅锌', grades: { Pb: 2.83, Zn: 3.57, Ag: 98.2 } },
        { mineral: '铜', grades: { Cu: 8.56, Ag: 45.3 } },
        { mineral: '铁', grades: { TFe: 15.68, mFe: 12.85 } },
      ],
      '玉龙铜业': [
        { mineral: '铜钼', grades: { Cu: 12.53, Mo: 0.86 } },
        { mineral: '铁', grades: { TFe: 18.52, mFe: 15.28 } },
      ],
      '鑫源矿业': [
        { mineral: '铅锌', grades: { Cu: 1.85, Pb: 3.26, Zn: 4.28, Ag: 68.5 } },
      ],
      '新疆瑞伦': [
        { mineral: '铜镍', grades: { Cu: 10.0, Ni: 2.83, Co: 0.56 } },
      ],
      '哈密博伦': [
        { mineral: '', grades: { V2O5: 3.58 } },
        { mineral: '铁', grades: { TFe: 24.56, mFe: 19.83 } },
      ],
      '肃北博伦': [
        { mineral: '铁', grades: { TFe: 18.53, mFe: 15.26 } },
      ],
      '双利矿业': [
        { mineral: '铁', grades: { TFe: 16.85, mFe: 13.82, Cu: 0.85, Pb: 1.23, Zn: 1.56 } },
      ],
      '大梁矿业': [
        { mineral: '铅锌', grades: { Pb: 5.28, Zn: 4.56, Ag: 85.3 } },
      ],
      '鸿丰伟业': [
        { mineral: '铁', grades: { TFe: 14.25, mFe: 11.86, Cu: 0.68, Zn: 1.25 } },
      ],
    };
    
    const minerals = unitMinerals[unit] || [{ mineral: '未知', grades: {} }];
    ...
    return minerals.map(item => ({
      ...
      grades: item.grades, // 品位信息
    }));
  };

  const [mineralDataList, setMineralDataList] = useState(generateMineralData(mineUnits[0]));

  // 地图悬停的矿山索引（用于显示弹窗和连接线）
  const [hoveredMineIndex, setHoveredMineIndex] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    const series: any[] = mineralDataList.map((data, idx) => {
      const labelFontSize = mineralCount === 1 ? 11 : (mineralCount === 2 ? 9 : 8);
      return {
        type: 'pie',
        radius: pieSize,
        center: centers[idx],
        avoidLabelOverlap: 'shift',
        label: {
          show: true,
          formatter: (params: any) => {
            return `${params.name}\n${params.value.toFixed(2)}万吨`;
          },
          color: '#7dd3fc',
        ...
        labelLine: {
          show: true,
          length: mineralCount === 1 ? 20 : (mineralCount === 2 ? 12 : 8),
          length2: mineralCount === 1 ? 15 : (mineralCount === 2 ? 10 : 6),
          lineStyle: {
        ...
        data: [
          { value: data.control, name: '控制', itemStyle: { color: '#6366f1' } },
          { value: data.infer, name: '推断', itemStyle: { color: '#06b6d4' } },
    ...
    const option: echarts.EChartsOption = {
      ...
      tooltip: { 
        trigger: 'item',
        formatter: (params: any) => {
          return `${params.name}: ${params.value.toFixed(2)}万吨 (${params.percent.toFixed(1)}%)`;
        }
      },
      legend: { show: false },
      series: series
  ...
  useEffect(() => {
    ...
    const geoOption: echarts.EChartsOption = {
      ...
      series: [
        ...
        // 连接线 - 只在悬停时显示
        ...(hoveredMineIndex !== null ? [{
          type: 'lines' as const,
          coordinateSystem: 'geo',
          zlevel: 2,
          data: [{
            coords: [minePoints[hoveredMineIndex]?.value || [100, 35], xining],
          }],
          effect: {
            show: true,
            period: 2,
        ...
        }] : []),
        {
          ...
          data: leftMines.map(p => ({ 
            ...
            value: p.value,
            itemStyle: { 
              color: hoveredMineIndex !== null && minePoints.indexOf(p) === hoveredMineIndex ? '#fbbf24' : '#f472b6',
              borderColor: hoveredMineIndex !== null && minePoints.indexOf(p) === hoveredMineIndex ? '#fff' : 'transparent',
              borderWidth: hoveredMineIndex !== null && minePoints.indexOf(p) === hoveredMineIndex ? 2 : 0
            }
          })),
          symbolSize: p => hoveredMineIndex !== null && minePoints.indexOf(p) === hoveredMineIndex ? 14 : 10,
          showEffectOn: 'render',
    ...
    fetch('./china.json')
      ...
      .then(geoJson => {
        ...
        // 添加鼠标事件监听 - 悬停时显示弹窗和连接线
        chart.on('mouseover', function(params) {
          if (params.seriesType === 'effectScatter' && params.name !== '西宁') {
            const mineIndex = minePoints.findIndex(p => p.name === params.name);
            if (mineIndex !== -1) {
              setHoveredMineIndex(mineIndex);
            }
          }
        });
      ...
      <div className="dashboard-body">
        ...
        <div className="dashboard-col col-left">
          <div className="module-card" style={{ flex: 0.9, minHeight: 160 }}>
            ...
            <div className="mining-rights-list-header">
              ...
              <div className="mining-rights-col col-area">规模</div>
          ...
          >
            ...
            {/* 合计数值和品位显示 */}
            <div className="pie-total-container">
              {mineralDataList.map((data, idx) => {
                const total = data.control + data.infer + data.prove;
                return (
                  <div key={idx} className="pie-total-item">
                    <div className="pie-total-name">{data.mineral}</div>
                    <div className="pie-total-value">{total.toFixed(2)}</div>
                    <div className="pie-total-unit">万吨</div>
                    {/* 品位信息 - 只显示红色标记的元素 */}
                    {data.grades && Object.keys(data.grades).length > 0 && (
                      <div className="pie-grades">
                        {Object.entries(data.grades).map(([element, grade]) => (
                          <span key={element} className="pie-grade-item">
                            <span className="pie-grade-element">{element}</span>
                            <span className="pie-grade-value">{grade}%</span>
                          </span>
                        ))}
                      </div>
                    )}
        ...
        <div className="dashboard-col col-center">
          ...
          <div className="module-card map-container" style={{ flex: 1.2, minHeight: 0, padding: 0, position: 'relative' }}>
            ...
            {/* 矿山详细信息弹窗 */}
            {(() => {
              if (hoveredMineIndex === null) return null;
              
              const currentMine = minePoints[hoveredMineIndex];
              const details = mineDetails[currentMine.name];
